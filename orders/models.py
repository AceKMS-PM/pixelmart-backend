...
class Order(TimeStampedModel):
    STATUS_CHOICES = [
        ('pending',    'Pending'),
        ('confirmed',  'Confirmed'),   # payment received, awaiting fulfilment
...
class OrderItem(models.Model):
    ...
    Inherits from plain Model (not TimeStampedModel) — order items are
    immutable once created; created_at / updated_at are not needed and
    would add noise.
    uuid is added explicitly as the public identifier (consistent with the
    rest of the project).
    """
    uuid      = models.UUIDField(default=None, editable=False, unique=True)   # set in save()
...
class Payout(TimeStampedModel):
    ...
    # Set when the payout processing service creates the ledger entry
